...
"Wcl Warning: %s.%s.wcCreate resource missing - need class or constructor",
    ...
    */
    if	( ( (NULLQUARK != rp->create.quark) && (NULL != 
	   (Constructor = WcMapConstructorFind( app, (intptr_t)rp->create.quark )) ))
	||( (NULLQUARK != rp->constructor.quark) && (NULL !=
	   (Constructor = WcMapConstructorFind( app, (intptr_t)rp->constructor.quark)) ))
	)
    {
    ...
    */
    if	( ( (NULLQUARK != rp->create.quark) && (NULL != 
	   (class = WcMapClassFind( app, (intptr_t)rp->create.quark )) ))
	||( (NULLQUARK != rp->class.quark) && (NULL !=
	   (class = WcMapClassFind( app, (intptr_t)rp->class.quark )) ))
	||( rp->className.quark && (NULL !=
	   (class = WcMapClassFind( app, (intptr_t)rp->className.quark )) ))
	)
    {
...
"Wcl Error: WcWidgetCreation(%s) - Failed \n\
    ...
    Possible: Resource file not found \n\
		user environment: \n\
			XENVIRONMENT is not set (examine $AMBERHOME/bin/xleap), \n\
			XAPPLRESDIR not terminated with '/' \n\
		user environment or installation: \n\
			$AMBERHOME/dat/XaLeap_wcl does not exist (reinstall leap). \n\
