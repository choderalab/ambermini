    ...
    WcLateBind	lb;
{
	...
	*/
	cbRecPtr = WcMapCallbackMethodFind( lb->app, (intptr_t)lb->classQ, (intptr_t)lb->nameQ );
    }
    else
    {
	/* Support historical brain fade, callbacks are NOT case sensitive
	*/
	cbRecPtr = WcMapCallbackFind( lb->app, (intptr_t)lb->nameq );
    }
    ...
    XrmQuark classQ;	/* quarkified case sensitive class name */
{
    ...
    do
    {
	object = WcMapObjectFind( w, (intptr_t)classQ );
    }
