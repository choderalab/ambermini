...
#define SuperClass ((ConstraintWidgetClass)&containerClassRec)
...
static XrmQuark	XtQChainLeft, XtQChainRight, XtQChainTop,
		XtQChainBottom, XtQRubber, XtQChainForm;
...
#define IS_EQUAL(quark) (q == quark)

    if      (IS_EQUAL(XtQChainLeft)   ) 
      { edge = XtChainLeft;   done(&edge, XtEdgeType); }
    else if (IS_EQUAL(XtQChainRight)  ) 
      { edge = XtChainRight;  done(&edge, XtEdgeType); }
    else if (IS_EQUAL(XtQChainTop)    ) 
      { edge = XtChainTop;    done(&edge, XtEdgeType); }
    else if (IS_EQUAL(XtQChainBottom) ) 
      { edge = XtChainBottom; done(&edge, XtEdgeType); }
    else if (IS_EQUAL(XtQRubber)      )
      { edge = XtRubber;      done(&edge, XtEdgeType); }
    else if (IS_EQUAL(XtQChainForm)   )
      { edge = XtChainForm;   done(&edge, XtEdgeType); }
...
static void ClassInitialize()
{
    ...
    XtQChainForm   = XrmStringToQuark("chainform");
    ...
    Widget w;
{
	...
	for (childP = children; childP - children < num_children; childP++) {
	    ...
	    if (form->form.right == XtChainForm)
	      form->form.virtual_width = fw->core.width - x -
		fw->form.default_spacing - 2 * (*childP)->core.border_width;
	    else
	    ...
	    if (form->form.bottom == XtChainForm)
	      form->form.virtual_height = fw->core.height - y -
		fw->form.default_spacing - 2 * (*childP)->core.border_width;
            else
