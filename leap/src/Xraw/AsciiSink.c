...
#ifdef GET_LAST_POS
#undef GET_LAST_POS		/* We will use our own GET_LAST_POS. */
#endif

#define GET_LAST_POS XawTextSourceScan(source, (XawTextPosition)0, XawstAll, XawsdRight, 1, TRUE)
...
XawTextPosition *leftPos, *rightPos;
{
    ...
    if (*leftPos > GET_LAST_POS)
      *leftPos = GET_LAST_POS;
    *rightPos = *leftPos;
}
...
Widget current, request, new;
{
  ...
  if (w->text_sink.font != old_w->text_sink.font
    ...
    XtReleaseGC(new, w->ascii_sink.normgc);
    XtReleaseGC(new, w->ascii_sink.invgc);
    GetGC(w);
