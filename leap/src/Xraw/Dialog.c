 ...
 */
	  ...
	  XtVaSetValues(w->dialog.valueW, XtNstring, w->dialog.value, NULL);
...
Widget w;
{
	...
	Widget *childP;
		...
		XtIsSubclass(*childP, commandWidgetClass)) {
	      ((DialogConstraints)
		 ((*childP)->core.constraints))->form.vert_base =
		   dw->dialog.valueW;
	    }
	}
    }
...
void
XawDialogAddButton(Widget dialog, char* name, XtCallbackProc function,
		   XtPointer param)
{
/*
