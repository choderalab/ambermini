     ...
     XEvent *event;
{
  ...
  /* VTR */
  if (ctx->text.s.type == XawselectPosition && ctx->text.s.left < ctx->text.s.right) 
    _XawTextZapSelection( ctx, event, TRUE);
  else
...
Cardinal *num_params;
{
  ...
  StartAction(ctx, event);
if (*num_params == 0) {
fprintf(stderr, "*num_params == 0\n");
exit(1);
}
  for (i = *num_params; i; i--, params++) {
    unsigned char hexval;

    text.length = strlen(*params);
    if (text.length == 0) 
	continue;
    text.ptr = *params;

    /* handle hex string 0x123456789abcdef (?) */
    if (text.length > 2  &&
        (*params)[0] == '0' && 
        (*params)[1] == 'x' && (*params)[2] != '\0') {
		char c, *p;
