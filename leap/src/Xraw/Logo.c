    ...
    Region region;		/* unused */
{
	...
	XmuDrawLogo(XtDisplay(w), XtWindow(w), w->logo.foreGC, w->logo.backGC,
		  w->simple.shadow_thickness, w->simple.shadow_thickness, 
		  (unsigned int) w->core.width - 2 * w->simple.shadow_thickness,
		  (unsigned int) w->core.height - 2 * w->simple.shadow_thickness);
    }
}
