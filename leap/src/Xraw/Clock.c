...
int tick_units;
{
	...
	ws = width * sinangle;
	SetSeg(w,
	       x = w->clock.centerX + l_round(length * sinangle),
	       y = w->clock.centerY - l_round(length * cosangle),
	       x1 = w->clock.centerX - l_round(ws + wc), 
	       y1 = w->clock.centerY + l_round(wc - ws));  /* 1 ---- 2 */
	/* 2 */
	SetSeg(w, x1, y1, 
	       x2 = w->clock.centerX - l_round(ws - wc), 
	       y2 = w->clock.centerY + l_round(wc + ws));  /* 2 ----- 3 */
...
int tick_units;
{
	...
	SetSeg(w,
	       x = w->clock.centerX + l_round(length * sinangle),
	       y = w->clock.centerY - l_round(length * cosangle),
	       w->clock.centerX + l_round(ms - wc),
	       w->clock.centerY - l_round(mc + ws) );
	SetSeg(w, w->clock.centerX + l_round(offset *sinangle),
	       w->clock.centerY - l_round(offset * cosangle), /* 2-----3 */
	       w->clock.centerX + l_round(ms + wc), 
	       w->clock.centerY - l_round(mc - ws));
