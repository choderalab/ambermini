...
static void _VendorShellClassInitialize(),Resize();
     ...
     Widget wid;
{
  ...
  for (i = w->composite.num_children, childP = w->composite.children; i; i--){
    if (XtIsManaged(*childP) && !XtIsSubclass(*childP,shellWidgetClass)) {
     ...
     Widget w;
{
  ...
  Widget *child = sw->composite.children;
  int i;

  for(i = 0; i < sw->composite.num_children; i++, child++) {
    if (XtIsManaged(*child) && !XtIsSubclass(*child,shellWidgetClass)) {
      XtResizeWidget( *child, sw->core.width, sw->core.height,
