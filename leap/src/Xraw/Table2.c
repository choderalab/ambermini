     ...
     XrmValue *value;
{
  ...
  if (tw->table.encoding)
    tw->table.literal_width = XTextWidth16(tw->table.font, (XChar2b*)"mmm", 3);
  else
...
/*
  commenting this out because, according to Purify, EDIT(w)->core has been freed
    ...
    char* raw;
{
  ...
  for ( s = raw, n = 0, length = 0;
        (*s) != '\0' && length < MAXSTRINGLENGTH;
        ++s, ++length )
  {
    if (isprint(*s))
      ++n;
  ...
  {
    char message[80];
    sprintf(message, "String of maximum length, %d; it may have been truncated",
            MAXSTRINGLENGTH );
    XtWarning( message );
    *s = '\0';
  }
...
void
XawTableDoLayout (Widget w, Boolean do_layout)
{
  XawTableWidget tw = (XawTableWidget)w;
...
int
XawTableSetNewSize (Widget w, int rows, int columns)
{
