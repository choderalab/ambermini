...
static void InitializeSearchWidget(), SetResource(), SetXawTableSearchLabels();
...
caddr_t call_data;		
{
  ...
  SetXawTableSearchLabels(search, SEARCH_LABEL_1, SEARCH_LABEL_2, FALSE);
}
...
struct SearchAndReplace * search;
{
  ...
  SetXawTableSearchLabels(search, msg, "", TRUE);
  return(FALSE);
}
...
Boolean once_only, show_current;
{
  ...
  while (TRUE) {
	...
	SetXawTableSearchLabels(search, "Selection has been modified, aborting.",
			"", TRUE);
	return(FALSE);
      }
      if (pos == end_pos) 
    ...
    if (XawTextReplace(tw, pos, end_pos, &replace) != XawEditDone) {
      ...
      SetXawTableSearchLabels(search, "*** Error while replacing", msg, TRUE);
      return(FALSE);
    }      
...
/*	Function Name: SetXawTableSearchLabels
...
static void
SetXawTableSearchLabels(search, msg1, msg2, bell)
struct SearchAndReplace * search;
String msg1, msg2;
Boolean bell;
{
...
Cardinal * num_params;
{
  ...
  if (*num_params != 1) {
    SetXawTableSearchLabels(search, "*** Error: SetField Action must have",
  ...
  case 'R':
    ...
    break;
  default:
    SetXawTableSearchLabels(search, "*** Error: SetField Action's first Argument must",
