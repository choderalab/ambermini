...
#if 0     
extern void bcopy Xraw_PROTO(( char* , char* , int ));
extern void bzero  Xraw_PROTO(( char *, int ));
#endif
...
static void VJump(), HScroll(), HJump(), ClearWindow(); 
...
TextWidget ctx;
{
  ...
  cursor = XCreateFontCursor(XtDisplay(ctx),XC_left_ptr);

  XtSetArg(args[n], XtNcursor, cursor); n++;
  ctx->text.vbar = vbar =
    XtCreateWidget("vScrollbar", scrollbarWidgetClass, (Widget)ctx, args, n);

  XtAddCallback( vbar, XtNdragProc,         VJump, (caddr_t)ctx );
  XtAddCallback( vbar, XtNvalueChangedProc, VJump, (caddr_t)ctx );
...
TextWidget ctx;
{
  ...
  int n = 0;
  Widget hbar;
  Cursor cursor = XCreateFontCursor(XtDisplay(ctx),XC_left_ptr);
  ...
  XtSetArg(args[n], XtNcursor, cursor); n++;
  ctx->text.hbar = hbar =
    XtCreateWidget("hScrollbar", scrollbarWidgetClass, (Widget)ctx, args, n);

  /*
  XtAddCallback( hbar, XtNscrollProc, HScroll, (caddr_t)ctx );
  XtAddCallback( hbar, XtNjumpProc, HJump, (caddr_t)ctx );
*/
  XtAddCallback( hbar, XtNdragProc,         HJump, (caddr_t)ctx );
  XtAddCallback( hbar, XtNvalueChangedProc, HJump, (caddr_t)ctx );
...
Cardinal *num_args;		/* unused */
{
  ...
  ctx->text.lastPos = (ctx->text.source != NULL) ? GET_LAST_POS : 0;
...
caddr_t closure, callData; /* closure = TextWidget, callData = percent. */
{
  ...
  scrollBarStruct = (XawScrollBarCallbackStruct*)callData;
...
caddr_t closure, callData; /* closuer = TextWidget, callData = percent. */
{
  ...
  XawTextLineTable * lt = (XawTextLineTable*)&(ctx->text.lt);
  XawScrollBarCallbackStruct *scrollBarStruct;
  float percent;
...
Widget w;
{
  ...
  ctx->text.update_disabled = False;
  lastPos = ctx->text.lastPos = GET_LAST_POS;
