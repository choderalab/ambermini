...
#include "../Xmu/CharSet.h"		/* for XmuCompareISOLatin1() */
#include "../Xmu/Misc.h"		/* for Min */
...
#define poff(field) XtOffsetOf(PannerRec, panner.field)
    ...
    { XtNshadowThickness, XtCShadowThickness, XtRDimension, sizeof(Dimension),
	XtOffsetOf(PannerRec, simple.shadow_thickness), XtRImmediate,
	(XtPointer) 2 },
...
static Bool DistinguishableColors (colors, count)
     XColor	*colors;
     int	count;
{
...
static Bool DistinguishablePixels (dpy, cmap, pixels, count)
     Display	    *dpy;
     Colormap	    cmap;
     ...
     int count;
{
    ...
    ret = DistinguishableColors (defs, count);
    free ((char *) defs);
    return ret;
}

#define XmuDistinguishableColors DistinguishableColors
#define XmuDistinguishablePixels DistinguishablePixels
...
#define FREE_PIXMAP(w,pm) XFreePixmap (XtDisplay(w), pm)

#else

#define BACKGROUND_STIPPLE(pw) \
  XmuCreateStippledPixmap (XtScreen((Widget)pw), pw->panner.shadow_color, \
			   pw->core.background_pixel, \
			   pw->core.depth)

#define FREE_PIXMAP(w,pm) XmuReleaseStippledPixmap (XtScreen(w), pm)

#endif /* defined(XtSpecificationRelease) && XtSpecificationRelease > 5 */
...
#if 0    
    XGCValues values;
    XGetGCValues(XtDisplay(gw), pw->panner.shadow_gc, GCTile, &values);
    XmuReleaseStippledPixmap(XtScreen(gw), values.tile);
#endif
    ...
    Region region;
{
    ...
    if (pw->simple.shadow_thickness)
    {
      XawDrawFrame (gw,
		    pw->simple.highlight_thickness,
		    pw->simple.highlight_thickness,
	     (Dimension)(pw->core.width - 2*pw->simple.highlight_thickness),
	     (Dimension)(pw->core.height - 2*pw->simple.highlight_thickness),
		    XawSUNKEN,
		    pw->simple.shadow_thickness,
		    pw->simple.top_shadow_GC,
		    pw->simple.bottom_shadow_GC);
