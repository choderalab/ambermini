...
#define DOUBLE_CLICK_DELAY ((Time)300)
...
static XtResource resources[] = {
    ...
    },
    {
      ...
      offset(list.double_callback), XtRCallback, NULL
    },
};
...
int *item;
{
    ...
    if (   (xloc < lw->list.internal_width)
	|| (yloc < lw->list.internal_height)
	|| (lw->core.width <= 2 * lw->list.internal_width)
	|| (lw->core.height <= 2 * lw->list.internal_height)){

      *item = 0;
      return(OUT_OF_RANGE);

    }else if((xloc > lw->core.width - lw->list.internal_width)
	     || (yloc > lw->core.height - lw->list.internal_height)){
    ...
    row = (yloc - (int) lw->list.internal_height) / lw->list.row_height;
    if ( row >= lw->list.nrows )
      row = lw->list.nrows - 1;
    
    col = (xloc - (int) lw->list.internal_width) / lw->list.col_width;
    if ( col >= lw->list.ncols )
      col = lw->list.ncols - 1;
    
    if (lw->list.vertical_cols) {
      *item = col * lw->list.nrows + row;
    }
    else {
      *item = row * lw->list.ncols + col;
    }
...
int *ul_ret, *lr_ret;
{
    ...
    (void)CvtToItem(w, xloc, yloc, ul_ret);
    xloc += event->xexpose.width;
    yloc += event->xexpose.height;
    (void)CvtToItem(w, xloc, yloc, lr_ret);
}
...
GC gc;
{
  ...
  width = (lw->core.width - 2 * lw->list.internal_width)/lw->list.ncols;
  width += lw->list.column_space;
...
/* ARGSUSED */
static void DelayProc (client_data, id)
    XtPointer client_data;
    XtIntervalId *id;      /* unused */
{
  Widget w = (Widget)client_data;

  Notify(w, (XEvent *)NULL, (String *)NULL, 0);
}
     ...
     Cardinal *num_params;
{
  ...
  if (id == (XtIntervalId)NULL  && event != (XEvent *)NULL)
  {
    id = XtAppAddTimeOut (XtWidgetToApplicationContext (w),
			  (unsigned long) DOUBLE_CLICK_DELAY,
			  DelayProc, (XtPointer)w);
    x = event->xbutton.x;
    y = event->xbutton.y;

    return;
  }
  else if (id == (XtIntervalId)NULL && event == (XEvent *)NULL)
  {
    return;
  }
  else if (id != (XtIntervalId)NULL && event == (XEvent *)NULL)
  ...
  {
    id = (XtIntervalId)NULL;
    callback_name = XtNdoubleClickCallback;
  ...
  if (XtCallbackHasSome == XtHasCallbacks(w, callback_name))
  {
    ...
    XtCallCallbacks( w, callback_name, (XtPointer) &ret_value);
  }
}
...
XawListShowCurrent(Widget w)
{
    ...
    ret_val = XtNew(XawListReturnStruct);
