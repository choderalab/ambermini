...
#  include <X11/Xraw/AsciiText.h> /* for Widget Classes. */
#endif
 ...
 */
    ...
    if (end_piece->used != 0)
      (void) MyStrncpy(end_piece->text, (end_piece->text + endPos - end_first),
		(int) end_piece->used);
  }
	...
	RemovePiece(src, start_piece);
      ...
      (void) MyStrncpy(start_piece->text + (startPos - start_first),
 ...
 */
  ...
  case XawstAll:
    /* EMPTY */
    break;                     /* ---- handled in special code above */
  }
...
XawTextBlock *        text;
{
  ...
  buf = (char*)XtCalloc((Cardinal)text->length,
...
String string, name;
{
  ...
  if ( ((fd = creat(name, 0666)) == -1 ) ||
    ...
    sprintf((char *)buf, "Error, while attempting to write to the file %s.",
	    name);
    XtAppWarning(XtWidgetToApplicationContext((Widget) src), (String)buf); 
    return(FALSE);
  }

  if ( close(fd) == -1 ) {
    sprintf((char *)buf, "Error, while attempting to close the file %s.",
	    name);
    XtAppWarning(XtWidgetToApplicationContext((Widget) src), (String)buf); 
    return(FALSE);
  }
...
AsciiSrcObject src;
{
  ...
  string = (String)XtCalloc((Cardinal)(src->ascii_src.length + 1),
		    (Cardinal)sizeof(unsigned char));
  
  for (first = 0, piece = src->ascii_src.first_piece ; piece != NULL; 
       first += piece->used, piece = piece->next) 
    strncpy((char*)(string + first), piece->text, (int) piece->used);
...
char * string;
{
  ...
  if (string == NULL) {
    if (src->ascii_src.type == XawAsciiFile) {
      local_str = (char*)XtCalloc((Cardinal)(src->ascii_src.length + 1),
				  (Cardinal)sizeof(unsigned char));
      freeit = 1;
      if (src->ascii_src.length != 0) {
	fseek(file, (long)0L, (int)0);
 ...
 */
  ...
  do {
    ...
    piece->text = (char*)XtCalloc((Cardinal)src->ascii_src.piece_size,
				  (Cardinal)sizeof(unsigned char));
    piece->allocated_string = TRUE;
...
Piece * piece;
{
  ...
  if (piece->allocated_string)
    XtFree(piece->text);

  XtFree((caddr_t)piece);
}
     ...
     int n;
{
  ...
  else if (((intptr_t)s1+n) < (intptr_t)s2 || ((intptr_t)s2+n) < (intptr_t)s1)
  {
    strncpy(s1, s2, n);
  }
  else if (((intptr_t)s1+n) >= (intptr_t)s2 && (intptr_t)s2 > (intptr_t)s1) 
  {
    register int i;
    for (i = 0; i < n; i++) {
      s1[i] = s2[i];
  ...
  else if (((intptr_t)s2+n) >= (intptr_t)s1 && (intptr_t)s1 > (intptr_t)s2) 
  {
    register int i;
    for (i = n-1; i >= 0; i--) {
      s1[i] = s2[i];
