...
contains
  ...
  subroutine qm2_diagonalizer_setup(diag_routine, allow_pseudo_diag, &
    ...
    if (diag_routine == 0) then
       ...
       if (master) then
          if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Auto diagonalization routine selection is in use.'')') ! lam81
          if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM:'')') ! lam81
       ...
       if (master .and. qmmm_struct%abfqmmm /= 1) &  ! lam81
    ...
    if (diag_routine == 1) then
       ...
       if (master) then 
          ...
          if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using internal diagonalization routine (diag_routine=1).'')') ! lam81
       ...
       if (master) then
          ! Only master does diagonalisation
          if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using dspev routine (diag_routine=2).'')') ! lam81
         ...
         if (master) then
            ...
            REQUIRE(ier==0)
            if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using dspevd routine (diag_routine=3).'')') ! lam81
            if (verbosity >= 2) then
              if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: Calling dspevd to query required scratch array sizes.'')') ! lam81
            end if
            ...
            if (verbosity >= 2) then
               if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dspevd required REAL scratch = '',i12,'' REALS.'')') & ! lam81
                    qmmm_scratch%lapack_dc_real_scr_aloc
               if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dspevd required INTEGER scratch = '',i12,'' INTEGERS.'')') & ! lam81
...
!            qmmm_scratch%lapack_dc_int_scr_aloc=3+5*qm2_struct%norbs
         ...
         if (master) then 
            ! Only master does diagonalisation
            if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using dspevx routine (diag_routine=4).'')') ! lam81
            allocate (qmmm_scratch%mat_diag_workspace(qm2_struct%norbs,1),stat=ier)
            REQUIRE(ier==0)
         ...
         if (master) then !Only master does diagonalisation
            allocate (qmmm_scratch%mat_diag_workspace(qm2_struct%norbs,1),stat=ier)
            REQUIRE(ier==0)
            if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using dsyev routine (diag_routine=5).'')') ! lam81
            if (verbosity >= 2) then
               if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: Calling dsyev to query required scratch array sizes.'')') ! lam81
            end if
            ...
            qmmm_scratch%lapack_dc_int_scr_aloc=0
            if (verbosity >= 2) then
               if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dsyev required REAL scratch = '',i12,'' REALS.'')') & ! lam81
                                                 qmmm_scratch%lapack_dc_real_scr_aloc
            end if
            allocate (qmmm_scratch%lapack_dc_real_scr(qmmm_scratch%lapack_dc_real_scr_aloc),stat=ier)
            REQUIRE(ier==0)
          ...
          if (master) then
             ...
             allocate (qmmm_scratch%mat_diag_workspace(qm2_struct%norbs,1),stat=ier)
             REQUIRE(ier==0)
             if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using dsyevd routine (diag_routine=6).'')') ! lam81
             if (verbosity >= 2) then
               if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: Calling dsyevd to query required scratch array sizes.'')') ! lam81
             end if
             ...
             qmmm_scratch%lapack_dc_real_scr_aloc = int(aloc_real_scr)
             if (verbosity >= 2) then
                if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dsyevd required REAL scratch = '',i12,'' REALS.'')') & ! lam81
                     qmmm_scratch%lapack_dc_real_scr_aloc
                if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dsyevd required INTEGER scratch = '',i12,'' INTEGERS.'')') & ! lam81
                     qmmm_scratch%lapack_dc_int_scr_aloc
             end if
             allocate (qmmm_scratch%lapack_dc_real_scr(qmmm_scratch%lapack_dc_real_scr_aloc),stat=ier)
             REQUIRE(ier==0)
          ...
          if (master) then !Only master does diagonalisation
             allocate (qmmm_scratch%mat_diag_workspace(qm2_struct%norbs,1),stat=ier)
             REQUIRE(ier==0)
             if(qmmm_struct%abfqmmm /= 1) write(6,'(''| QMMM: Using dsyevr routine (diag_routine=7).'')') ! lam81
!            if (verbosity >= 2) then
...
!            qmmm_scratch%lapack_dc_real_scr_aloc = int(aloc_real_scr)
            ...
            if (verbosity >= 2) then
              if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dsyevr required REAL scratch = '',i12,'' REALS.'')') & ! lam81
                                                 qmmm_scratch%lapack_dc_real_scr_aloc
              if(qmmm_struct%abfqmmm /= 1) write(6,'(''QMMM: dsyevr required INTEGER scratch = '',i12,'' INTEGERS.'')') & ! lam81
                                                 qmmm_scratch%lapack_dc_int_scr_aloc
            end if     
            allocate (qmmm_scratch%lapack_dc_real_scr(qmmm_scratch%lapack_dc_real_scr_aloc),stat=ier)
            REQUIRE(ier==0)
