...
#line 114 "cifparse.y"
  ...
  strncpy(TempKeyword,(yyvsp[(1) - (1)].TempBuffer),MxNameLen);
;}
    break;
...
#line 127 "cifparse.y"
  ...
  strncpy(TempValue,(yyvsp[(1) - (1)].TempBuffer),MAXVALUELENGTH);
;}
    break;
...
#line 142 "cifparse.y"
    {
  int idatablock;
  idatablock = ndb_cif_get_datablock_id(&((yyvsp[(1) - (1)].TempBuffer))[5]);
  if (idatablock == 0)
    ndb_cif_new_datablock(&((yyvsp[(1) - (1)].TempBuffer))[5]);
  else {
    ndb_cif_move_datablock(&((yyvsp[(1) - (1)].TempBuffer))[5]);
